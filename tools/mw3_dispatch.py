#!/usr/bin/env python3
"""Disassemble the dispatcher that installs the span-function pointers, find strings."""
...
out = open(r"C:\tmp\mw3_dispatch_out.txt", "w", encoding="utf-8")
...
def find_func_start(rva):
    """walk back for 55 8b ec preceded by ret/int3/nop."""
    off = rva - base
    i = off
    while i > 0 and off - i < 0x3000:
...
# function containing 0x557c76
fs = find_func_start(0x557c76)
p(f"Dispatcher containing 0x557c76: function starts at 0x{fs:08x}" if fs else "no start found")
start = fs if fs else 0x557b00
p("=" * 100)
code = data[start-base : 0x557f00-base]
for ins in md.disasm(code, start):
    p(fmt(ins))
    if ins.mnemonic == 'ret' and ins.address > 0x557ca8:
        break
out.close()
print("done")
